...
RSpec.describe SubParser::Timestamp do
  ...
  describe '#parse' do
    it 'should parse standard timestamps' do
      timestamp = SubParser::Timestamp.parse '01:02:03,004'

      expect(timestamp.hours).to eq 1
      expect(timestamp.minutes).to eq 2
      expect(timestamp.seconds).to eq 3
      expect(timestamp.milliseconds).to eq 4
    end

    it 'should parse one or more hour digits' do
      timestamp = SubParser::Timestamp.parse '1:02:03,004'
      expect(timestamp.hours).to eq 1

      timestamp = SubParser::Timestamp.parse '111:02:03,004'
      expect(timestamp.hours).to eq 111
    end
  end
